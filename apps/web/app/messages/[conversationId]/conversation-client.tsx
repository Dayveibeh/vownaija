      ...
      {quoteOpen && <div className="quote-builder-backdrop" onMouseDown={() => setQuoteOpen(false)}>
        <section className="quote-builder" role="dialog" aria-modal="true" aria-label="Create quote" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <form onSubmit={sendQuote}>
            ...
            <div className="quote-builder-adjustments"><label>Discount (₦)<input type="number" min="0" step="1000" value={discount || ""} onChange={(event) => setDiscount(Number(event.target.value))} placeholder="0" /></label><label>Additional fees (₦)<input type="number" min="0" step="1000" value={fees || ""} onChange={(event) => setFees(Number(event.target.value))} placeholder="0" /></label><label>Valid until<input type="date" value={validUntil} onChange={(event) => setValidUntil(event.target.value)} /></label></div>
